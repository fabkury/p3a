...
static inline uint32_t gif_millis(void)
{
    return (uint32_t)(esp_timer_get_time() / 1000ULL);
}

static inline void gif_delay(uint32_t ms)
{
    if (ms == 0) {
        return;
    }
...
#else
#define gif_millis millis
#define gif_delay delay
#endif
...
uint32_t lTime = gif_millis();
    ...
    {
#if !defined( __MACH__ ) && !defined( __LINUX__ ) 
        lTime = gif_millis() - lTime;
        if (lTime < _gif.iFrameDelay) // need to pause a bit
           gif_delay(_gif.iFrameDelay - lTime);
#endif // __LINUX__
    }
