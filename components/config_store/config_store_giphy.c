...
/**
 * @file config_store_giphy.c
 * @brief Config store Giphy-specific settings: API key, rendition, refresh interval
 */
...
// ============================================================================
// ESP-Hosted Transport-Failure Reboot Counters
// ============================================================================

#define NVS_KEY_TP_RST_TOT "tp_rst_tot"
#define NVS_KEY_TP_RST_STR "tp_rst_str"

uint16_t config_store_get_transport_reboot_total(void)
{
    return nvs_read_u16(NVS_KEY_TP_RST_TOT);
}

void config_store_increment_transport_reboot_total(void)
{
    uint16_t v = nvs_read_u16(NVS_KEY_TP_RST_TOT);
    if (v < UINT16_MAX) v++;
    nvs_write_u16(NVS_KEY_TP_RST_TOT, v);
}

uint16_t config_store_get_transport_reboot_streak(void)
{
    return nvs_read_u16(NVS_KEY_TP_RST_STR);
}

void config_store_increment_transport_reboot_streak(void)
{
    uint16_t v = nvs_read_u16(NVS_KEY_TP_RST_STR);
    if (v < UINT16_MAX) v++;
    nvs_write_u16(NVS_KEY_TP_RST_STR, v);
}

void config_store_reset_transport_reboot_streak(void)
{
    nvs_write_u16(NVS_KEY_TP_RST_STR, 0);
}

void config_store_reset_transport_reboot_counters(void)
{
    nvs_write_u16(NVS_KEY_TP_RST_TOT, 0);
    nvs_write_u16(NVS_KEY_TP_RST_STR, 0);
}
