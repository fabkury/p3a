/**
 * @file http_api_page_settings.c
 * @brief Settings configuration page
 * 
 * Contains handler for:
 * - GET /settings - Settings page (Background, Display, Storage, Seed)
 */
...
/**
 * GET /settings
 * Returns the settings configuration page
 */
esp_err_t h_get_settings(httpd_req_t *req)
{
    static const char html[] =
        ...
        "    </div>"
        "    <div class=\"toggle-row\">"
        "        <label for=\"viewack-toggle\" class=\"toggle-label\">Request View Acknowledgment</label>"
        "        <label class=\"toggle-switch\">"
        "            <input type=\"checkbox\" id=\"viewack-toggle\" onchange=\"toggleViewAck(this.checked)\">"
        "            <span class=\"toggle-slider\"></span>"
        "        </label>"
        "    </div>"
        "</div>"
        ...
        "    </div>"
        "    <div class=\"file-name\" id=\"sd-hint\">Folder path on SD card (e.g., /p3a, /data). Reboot required.</div>"
        "</div>"
        "<div class=\"section\">"
        "    <h3>Global Seed</h3>"
        "    <div class=\"seed-row\">"
        ...
        "    <button class=\"footer-btn\" onclick=\"reboot()\">Reboot</button>"
        "</div>"
        ...
        "                var resp = JSON.parse(xhr.responseText || '{}');"
        "                var cfg = (resp && resp.ok && resp.data) ? resp.data : {};"
        "                var viewAck = (typeof cfg.view_ack === 'boolean') ? cfg.view_ack : false;"
        "                document.getElementById('viewack-toggle').checked = viewAck;"
        "            } catch (e) {"
        "                document.getElementById('viewack-toggle').checked = false;"
        "            }"
        "        }"
        "    };"
        "    xhr.send();"
        "}"
        "function toggleViewAck(enable) {"
        "    var xhrGet = new XMLHttpRequest();"
        "    xhrGet.open('GET', '/config', true);"
        "    xhrGet.onreadystatechange = function() {"
        "        if (xhrGet.readyState === 4) {"
        ...
        "                if (xhrPut.readyState === 4) {"
        "                    if (xhrPut.status >= 200 && xhrPut.status < 300) {"
        "                        showToast('View acknowledgment ' + (enable ? 'enabled' : 'disabled'), false);"
        "                    } else {"
        "                        showToast('Save failed (HTTP ' + xhrPut.status + ')', true);"
        "                        document.getElementById('viewack-toggle').checked = !enable;"
        "                    }"
        "                }"
        "            };"
        "            xhrPut.send(JSON.stringify(cfg));"
