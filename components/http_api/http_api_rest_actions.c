...
/**
 * POST /action/debug/storage_eviction
 * Body: {"target_mib": <positive number>}
 * Forces storage_eviction to run with the given free-space target, bypassing
 * the configured CONFIG_STORAGE_EVICTION_TARGET_MIB and the SNTP guard.
 */
esp_err_t h_post_debug_storage_eviction(httpd_req_t *req)
{
    ...
    cJSON *t = cJSON_GetObjectItem(root, "target_mib");
    if (!t || !cJSON_IsNumber(t) || t->valuedouble <= 0) {
        cJSON_Delete(root);
        send_json(req, 400, "{\"ok\":false,\"error\":\"target_mib must be a positive number\",\"code\":\"BAD_TARGET\"}");
        return ESP_OK;
    }
    uint64_t target_bytes = (uint64_t)t->valuedouble * 1024ULL * 1024ULL;
    uint32_t target_mib   = (uint32_t)t->valuedouble;
    cJSON_Delete(root);

    uint64_t free_before = 0;
    storage_eviction_get_free_space(&free_before);

    esp_err_t err = storage_eviction_run_with_target_debug(target_bytes);

    uint64_t free_after = 0;
    storage_eviction_get_free_space(&free_after);

    char resp[192];
    snprintf(resp, sizeof(resp),
             "{\"ok\":%s,\"data\":{\"err\":%d,\"target_mib\":%lu,"
             "\"free_before_mib\":%llu,\"free_after_mib\":%llu}}",
             err == ESP_OK ? "true" : "false",
             (int)err,
             (unsigned long)target_mib,
             (unsigned long long)(free_before / (1024 * 1024)),
             (unsigned long long)(free_after  / (1024 * 1024)));
    send_json(req, 200, resp);
    return ESP_OK;
}

/**
 * POST /action/debug/channel_eviction
 * Runs channel_eviction_check_and_run() directly. Inherits the function's
 * normal guards (SNTP-sync required, age = CONFIG_CHANNEL_EVICTION_AGE_DAYS,
 * active-playset channels protected).
 */
esp_err_t h_post_debug_channel_eviction(httpd_req_t *req)
{
    ...
    esp_err_t err = channel_eviction_check_and_run();

    char resp[96];
    snprintf(resp, sizeof(resp),
             "{\"ok\":%s,\"data\":{\"err\":%d}}",
             (err == ESP_OK || err == ESP_ERR_INVALID_STATE) ? "true" : "false",
             (int)err);
    send_json(req, 200, resp);
    return ESP_OK;
}
