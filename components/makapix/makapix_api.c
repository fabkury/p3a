...
/**
 * @file makapix_api.c
 * @brief Makapix MQTT request/response API: channel queries, artwork lookup, playset sync
 */
...
static const char *channel_to_string(makapix_channel_type_t channel)
{
    ...
    case MAKAPIX_CHANNEL_REACTIONS: return "reactions";
...
static esp_err_t publish_and_wait(cJSON *request_obj, cJSON **out_response)
{
    ...
    // Wait for response topic subscription (max 5 seconds)
    ...
    // Heap-allocate pending request to avoid use-after-free if task is deleted
    // while blocked waiting for response (e.g., during channel switch)
    pending_request_t *pending = calloc(1, sizeof(pending_request_t));
    if (!pending) {
        free(payload);
        return ESP_ERR_NO_MEM;
    }
...
esp_err_t makapix_api_query_posts(const makapix_query_request_t *req, makapix_query_response_t *resp)
{
    ...
    if (req->channel == MAKAPIX_CHANNEL_BY_USER ||
        req->channel == MAKAPIX_CHANNEL_REACTIONS) {
    ...
    if (limit == 0) limit = 30;
    // Per spec: maximum pagination size is 48 artworks per query
...
esp_err_t makapix_api_get_playset(const char *playset_name, ps_playset_t *out_playset)
{
    ...
    cJSON_AddStringToObject(root, "request_type", "get_playset");
    cJSON_AddStringToObject(root, "playset_name", playset_name);
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "get_playset request failed: %s", esp_err_to_name(err));
        return err;
    }
    ...
    ESP_LOGI(TAG, "get_playset '%s': %zu channels",
