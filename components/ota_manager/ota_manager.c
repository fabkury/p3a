                                                ...
                                                ota_progress_cb_t progress_cb)
{
    ...
    // Download using HTTP client with chunked transfer
    ...
    ESP_LOGI(TAG, "Downloading %d bytes...", content_length);

    size_t total_read = 0;
    int read_len;
    size_t remaining = (size_t)content_length;
    while (total_read < remaining && total_read < max_size) {
        size_t to_read = max_size - total_read;
        if (to_read > CONFIG_OTA_HTTP_BUFFER_SIZE) {
            to_read = CONFIG_OTA_HTTP_BUFFER_SIZE;
        }
        ...
        if (progress_cb && content_length > 0) {
            int percent = (int)((total_read * 100) / content_length);
            progress_cb(percent, "Downloading web UI...");
        }
    }
    ...
    if (expected_sha256 && strlen(expected_sha256) == 64) {
        ...
        mbedtls_sha256_init(&ctx);
        mbedtls_sha256_starts(&ctx, 0);
        mbedtls_sha256_update(&ctx, buffer, total_read);
        mbedtls_sha256_finish(&ctx, computed);
        mbedtls_sha256_free(&ctx);
