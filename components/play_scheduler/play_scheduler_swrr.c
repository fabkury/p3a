...
static inline bool has_available_artwork(const ps_channel_state_t *ch)
{
    ...
    size_t entry_count = ((ch->entry_format == PS_ENTRY_FORMAT_MAKAPIX || ch->entry_format == PS_ENTRY_FORMAT_GIPHY || ch->entry_format == PS_ENTRY_FORMAT_INSTITUTION || ch->entry_format == PS_ENTRY_FORMAT_KLIPY) && ch->cache)
