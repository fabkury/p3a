...
// Define fbInfo structure here (before board_framebuffer.h uses it)
// This matches the original structure where fbInfo is defined in the driver file
typedef struct fbInfo {
	void *			pixels;			// The pixel buffer
	gCoord			linelen;		// The number of bytes per display line
	} fbInfo;

// Include board_framebuffer.h again after gdisp_driver.h to ensure board_init is visible
// with GDisplay fully defined. Undefine header guard to allow reprocessing.
// Note: board_init is static, so defining it twice with identical bodies should be fine.
#undef _BOARD_FRAMEBUFFER_H
...
/*===========================================================================*/
/* Custom extension: Update framebuffer pointer at runtime                   */
/*===========================================================================*/

void gdisp_lld_set_framebuffer(void *pixels, gCoord linelen) {
	GDisplay *g = gdispGetDisplay(0);
	if (g && g->priv) {
		((fbPriv *)g->priv)->fbi.pixels = pixels;
		((fbPriv *)g->priv)->fbi.linelen = linelen;
	}
}
...
#define FB_BYTES_PER_PIXEL		((LLDCOLOR_BITS + 7) / 8)
#define PIXIL_POS(g, x, y)		((y) * ((fbPriv *)(g)->priv)->fbi.linelen + (x) * FB_BYTES_PER_PIXEL)
#define PIXEL_ADDR(g, pos)		((uint8_t *)(((char *)((fbPriv *)(g)->priv)->fbi.pixels)+pos))
...
LLDSPEC	gColor gdisp_lld_get_pixel_color(GDisplay *g) {
	...
	memcpy(&color, PIXEL_ADDR(g, pos), FB_BYTES_PER_PIXEL);
