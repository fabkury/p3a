...
- **Boot restore**: Every `play_scheduler_execute_playset()` writes `{sd-root}/active_playset.bin` (root resolved at runtime via `sd_path_get_root()`, default `/sdcard/p3a`; full `ps_playset_t` blob, magic+version+CRC, atomic rename). On boot, `animation_player_restore_boot_playset()` loads that snapshot and re-executes it. A snapshot the scheduler installed counts as restored even if the immediate pick failed (the first-swap gates start playback once content is available); only a snapshot that cannot be loaded or installed falls back to channel_promoted, and that fallback runs with `persist=false` (`play_scheduler_execute_playset_ex()`) so it never overwrites the snapshot. The snapshot is never deleted on a load failure either. Because the snapshot lives under the configurable root, switching the root cold-starts the playset too.
- **Key concepts**:
...
## 10. klipy — Klipy Integration

- **Purpose**: Fetch and cache GIFs and stickers from the Klipy API
- **Key files**: `klipy_api.c`, `klipy_cache.c`, `klipy_download.c`, `klipy_refresh.c`
- **Features**:
  - Trending, search, and category channels, each for two products: GIF and sticker
  - Hash-sharded file storage on SD card (`/sdcard/p3a/klipy/{gif|sticker}/`)
  - Klipy CDN URLs are opaque/non-guessable, so cache entries store the numeric Klipy id and re-resolve the download URL via the API when needed
  - Configurable format (GIF/WebP), content rating, refresh interval, and cache size (32–2500)
  - BYOK: no key is shipped; channels do not refresh until the user enters a key from [partner.klipy.com](https://partner.klipy.com/) (test keys: 100 requests/hour, ~50 items/request)
  - On-demand download with atomic writes (temp file + rename)
- **Web UI**: Klipy tab at `/settings#klipy`; channel creation in the playset editor
- **Kconfig**: `KLIPY_API_KEY_DEFAULT` (empty), `KLIPY_FORMAT_DEFAULT` (default `gif`), `KLIPY_RATING_DEFAULT` (default `pg-13`)
- **Playset binary format**: Klipy channels use `PS_CHANNEL_TYPE_KLIPY`

## 11. art_institution — Museum (IIIF) Channels

- **Purpose**: First-class channel source for artwork hosted by major museums that expose their collections through the [IIIF Image API](https://iiif.io/api/image/3.0/) or, for Cleveland and Minneapolis, fixed-rendition open-access image CDNs. Nine museums ship today: the Art Institute of Chicago (`artic`), the Rijksmuseum (`rijks`), the Victoria and Albert Museum (`vam`), the Wellcome Collection (`wellcome`), the Statens Museum for Kunst (`smk`), Harvard Art Museums (`ham`, BYOK API key), the Smithsonian (`si`, BYOK api.data.gov key), the Cleveland Museum of Art (`cma`), and the Minneapolis Institute of Art (`mia`).
- **Key files**: `art_institution.c` (dispatch + lifecycle), `art_institution_refresh.c` (per-channel listing walk), `art_institution_download.c` (IIIF JPEG fetch with NRS→IDS redirect shim), `art_institution_resolve.c` (Rijks Linked-Art walk), `art_institution_rate_limit.c` (per-museum cooldown), `museums/{artic,rijksmuseum,vam,wellcome,smk,ham,smithsonian,cma,mia}.c` (per-museum adapters), `museums/common.c` (shared HTTP helpers)
...
## 12. makapix — Makapix Club Integration
...
## 13. http_api — HTTP Server and REST API
...
## 15. ota_manager — Over-the-Air Updates
...
## 16. slave_ota — ESP32-C6 Co-processor Firmware
...
## 18. content_cache — Channel Cache Wrapper
...
- **Policy**: Two-watermark hysteresis with capacity-scaled watermarks. The trigger is `STORAGE_EVICTION_TARGET_PCT` of the card's total capacity clamped to [256 MiB, `STORAGE_EVICTION_TARGET_MIB`]; eviction then runs until free space reaches trigger + headroom, where headroom is `STORAGE_EVICTION_HEADROOM_PCT` of capacity clamped to [512 MiB, `STORAGE_EVICTION_HEADROOM_MIB`]. With defaults (10% / 25%, ceilings 1024 / 4096 MiB), cards of 32 GB and up reproduce the classic fixed behavior (trigger at 1 GiB free, evict until ~5 GiB free), while a 4 GB card gets ~378 MiB trigger / ~1.3 GiB stop — which is why **4 GB is the minimum SD card size** (16+ GB still recommended for a larger cache working set). The check runs after every download (success or failure), so the trigger engages as space shrinks rather than only after the first ENOSPC.
- **Small-card guard**: If the SD card's total capacity is below `STORAGE_EVICTION_MIN_CARD_SIZE_MIB` (default 1024 MiB, below which the clamped watermark floors would eat most of the card), eviction skips silently. The device fills the card and stops accepting new downloads rather than thrashing the FS. Set the Kconfig to 0 to disable the guard.
- **Full-card backoff**: When a download fails and free space is still below ~16 MiB after the eviction check (eviction disabled by the guard, or everything younger than the never-delete age floor), the download manager pauses download attempts for 10 minutes instead of burning a doomed TLS handshake per missing entry. It re-checks free space every minute and resumes early as soon as space frees up (see `download_manager.c`, `DL_FULL_FREE_BYTES`).
- **Kconfig**: `STORAGE_EVICTION_TARGET_PCT` (trigger, % of capacity, default 10), `STORAGE_EVICTION_TARGET_MIB` (trigger ceiling, default 1024 MiB), `STORAGE_EVICTION_HEADROOM_PCT` (overshoot above trigger, % of capacity, default 25), `STORAGE_EVICTION_HEADROOM_MIB` (headroom ceiling, default 4096 MiB), `STORAGE_EVICTION_MIN_CARD_SIZE_MIB` (small-card guard, default 1024 MiB), `STORAGE_EVICTION_INITIAL_AGE_DAYS` (default 30), `STORAGE_EVICTION_MIN_AGE_HOURS` (default 4), `CHANNEL_EVICTION_AGE_DAYS` (default 60)

## 20. loader_service — Animation File Loader
...
## 21. playback_queue — Play Scheduler to Animation Player Adapter
...
## 22. sdio_bus — SDIO Bus Coordinator
...
## 23. show_url — URL Artwork Downloader
...
## 24. debug_http_log — Performance Instrumentation
