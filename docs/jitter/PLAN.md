...
| H3b | **SD DMA bounce path (found 2026-08-28).** On the P4 `sdmmc_write/read_sectors` uses a PSRAM buffer for DMA only when address AND size are cache-line (128 B) aligned; otherwise it bounces through a 512 B internal buffer, one SD command per sector. `http_fetch_to_file`'s 32 KB chunk (plain `heap_caps_malloc` SPIRAM) and `loader_service`'s file buffer (size = file size) both fail the check: 64 commands per 32 KB write (72 ms median, 345 ms p99 in RUN-05), ~1000 commands per 500 KB artwork read (35 k 512-byte reads in 11 min). | `sd_write`/`sd_read` span marks: durations collapse (≥10x) and counts drop after aligning the buffers; upscale anomalies and stalls should follow if the bounce storm is the driver. A/B: RUN-06 (before) vs RUN-07 (after). |
| H3c (ROOT PHYSICS, 2026-08-30) | IDF `sdmmc_wait_for_idle()` polls CMD13 without yielding for up to 100 ms after every write; while the card is busy (1–45 ms per block) the host is hammered with hundreds of commands and both cores' decode/upscale crawl. Proven by substitution: a yielding wait makes the bounce reproducer stall-free (0 anomalies vs 6–9). Fix 8 = always-on `sd_idle_wait` wrapper. | `runs/RUN-20260830-03-04.md` |
| H3b (mechanism, 2026-08-29) | The bounce loop's `sdmmc_wait_for_idle()` polls CMD13 without yielding for up to 100 ms after every single-block write; with this card busy 1–35 ms per block, 64 blocks per 32 KB chunk saturate core 0 with host-controller churn and starve `upscale_top`. Reproduced on demand (`sd_experiment.py`, buf=0 chunk=32768) and absent for aligned/internal buffers and single-sector FATFS writes. | `runs/RUN-20260829-02-06-experiments.md` |
| H4 (closed 2026-08-29) | **PSRAM bandwidth saturation.** DPI DMA ~93 MB/s continuous from PSRAM + code fetch from PSRAM + SD DMA + 1.5 MB memset/upscale. | Producer time inflates uniformly (decode and upscale both) during I/O bursts with no lock contention; consumer wakes on time but the frame is not ready. |
