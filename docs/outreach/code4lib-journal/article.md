...
Word count ≈ 4,400 (journal guidance: 1,500–5,000).
...
artwork from seven museums: the Art Institute of Chicago, the
...
hardware is a Waveshare ESP32-P4 development board — $39.99 retail as
of August 2026; add a microSD card and any USB-C power source and the
whole build stays under $60. The ESP32-P4 is a dual-core 400 MHz
RISC-V microcontroller: no operating system beyond FreeRTOS, no
browser stack, 32 MB of PSRAM plus about 768 KB of internal SRAM, a
hardware JPEG decoder, and Wi-Fi via a companion chip. The board
carries a 4-inch, 720×720 IPS touchscreen. The firmware is C on
ESP-IDF. p3a began as a pixel-art player — it plays animations from
Giphy and Klipy, artworks from Makapix Club (a pixel-art community I
also run), and files from its SD card — and the museum feature grew
out of a search for more things worth putting on a good square screen.
...
variety of models (including Anthropic's) inside the Cursor IDE, later
with Claude Code, generally running the newest Opus-class model
...
article. Integrating one museum's images usually means reverse-
engineering one museum's image infrastructure; IIIF meant that
"integrate seven museums' images" was *one* engineering task. But only
for the images. Above the pixels — finding artworks, browsing
collections, paginating — no such layer exists in practice, and each
museum cost a separate adapter. One URL template, seven discovery
layers. The rest of this article walks both halves of that sentence.
...
p3a's unit of playback is a *channel*: a saved selection like "Art
Institute of Chicago → Departments → Modern and Contemporary Art" or
"Rijksmuseum → the curated set *Paintings*". A user assembles channels
in a small web app the device itself serves to a phone or laptop on
the same network; the firmware then keeps each channel stocked — it
periodically refreshes the artwork listing (every four days by
default), downloads images to a per-museum cache on the SD card
(1,024 artworks per channel by default), and rotates them on screen.
...
and streams the response to the SD card. That is the entire
...
success story of a kind I have rarely encountered, and the people who
wrote and implemented the Image API deserve to hear it.
...
p3a never does. The panel is 720×720, `!720,720` is universally
honored, and skipping negotiation halves the request count. In
fourteen months of operation this shortcut has not misfired once —
at display resolution, request-time sizing appears to be simply
sufficient. (Where advertisement and reality diverge, they diverge in
the *other* direction — see SMK, below.)

**JPEG only.** The ESP32-P4 has a hardware JPEG decoder, and museum
IIIF servers serve JPEG far more reliably than WebP. (Honesty
requires a caveat: progressive JPEGs and certain dimension
combinations fall back to a software decoder, carefully fenced so a
corrupt file cannot crash the device. "Hardware-decoded" is the happy
...
a 16 MiB cap guards against surprises. A client with 32 MB of total
RAM does not get to "just download the image and see".

## 4. The bespoke layer: seven discovery adapters

Above the Image API, each museum is its own country. The plainest
evidence is a line count of p3a's per-museum adapters — each
...
same API". What follows are the exhibits I would show a visitor —
each one a real behavior a client must handle, none of them
documented failures, all of them observed politely from outside.

### 4.1 Art Institute of Chicago: the thousand-record wall

AIC's is the best-documented museum API I have used, and the adapter
is still the longest. Two behaviors of its Elasticsearch-backed search
...
HTTP 403 past roughly page ten, independent of the first limit. The
adapter treats a mid-walk 403 as a partial success: keep what was
merged, skip cache eviction (records that merely went unfetched must
not be mistaken for records the museum withdrew), retry no sooner
than the next scheduled refresh.

Reaching *past* the wall took the adapter's one genuinely creative
...
this code and slightly embarrassed that it needs to exist.
...
fetch (with a manual redirect dance: the id-resolver host answers
with a 303 that the embedded HTTP client, under its streaming read
pattern, refuses to follow on its own).
...
tombstoned and skipped forever — until a future refresh re-lists the
artwork, which resets its budget. The cursor-only pagination also
means the "arbitrary offset" primitive is emulated: the client reads
the total from the stream's metadata, wraps the offset into range,
and pays ⌈N/100⌉ throwaway page fetches to reach it. Every cost in
this paragraph is the honest price of the standards-based path, and
...
Rijksmuseum's table of contents wherever it goes.
...
identify yourself (`p3a/{version}`, with a contact email), which is
exactly what a WAF wants of a well-behaved bot. The lesson
generalizes; the diagnosis, at 2 a.m., did not feel general.
...
to survive a single channel refresh, a fact the settings UI has to
explain to users.
...
become channels — a standards observation (stable short identifiers
are a kindness) delivered by a struct.
...
fields, so its metadata-rich pages must be swallowed whole — 512 KB
of buffer for fifty artworks.)

**The Victoria and Albert Museum** mostly just works — the shortest
adapter — but computes `count = 0` for one facet type whenever the
has-images filter is applied, so the browse layer enumerates that
facet unfiltered and re-probes each term individually to recover
honest counts.
...
Smithsonian's documentation does not mention Presentation at all.
Six of seven do serve lovely per-artwork *manifests* — but a manifest
is addressed by an object id that only the search API can supply, so
manifests presuppose discovery rather than provide it.
...
offset emulation, and three extra fetches per artwork. Reader, the
conclusion I did not expect to reach: p3a's split — bespoke search
API for discovery, IIIF for pixels — is not a pragmatic betrayal of
the standard. It is the division of labor the IIIF specifications
...
IIIF community, and — as the adoption findings above suggest —
unlikely to be closed by Presentation Collections as they stand.
...
turn. Without this, the user browsing Chicago's departments on a
phone would silently spend the refresh budget of the frame on the
shelf beside them.
...
concurrency; the whole device allows itself two TLS connections. A
full channel refresh costs roughly 41–82 API calls against daily
quotas of 1,000–2,500 where quotas exist — and runs every four days.

In candor, one designed mechanism was never built: a defensive
cooldown after repeated *connection* failures (as opposed to 429s)
exists in p3a's design document and nowhere in its code. Every system
carries a to-do list; articles that pretend otherwise are resented by
their own authors within the year.
...
image identifier. There is something clarifying about a budget this
tight — it converts vague API preferences into sharp, testable
requirements, and I offer the conversions as data points from the
small end of the client spectrum:
...
- **Identifier *stability* is one too.** The record survives
  firmware updates, cache rebuilds, and re-listings; identifiers
  that drift orphan their own files.
- **One byte of state is enough — barely.** The file-type byte
  ...
  ends up doing two jobs; the second job is where the bugs live.
...
7. **Document the sharp edges** — result-window caps, key-quota
   ...
   article was learned by a device in someone's home hitting it.
...
microcontroller firmware has been described. I make the claim in its
weakest form and would genuinely welcome correction; establishing
prior art would be a contribution to this article's subject, not a
blow to it.

## 9. Conclusion

The IIIF Image API kept a promise that standards rarely keep at this
scale: seven institutions, seven image stacks, one line of client
code. The discovery layer above it remains a per-museum negotiation —
not because museums ignored a standard, but because the standard
IIIF offers there is scoped for viewers and harvesters, not for
small clients asking small questions ("what's in this collection?
how many? give me 400–499"). A $40 microcontroller turns out to be a
usefully honest instrument for measuring that gap: everything
implicit becomes a buffer size, a request count, or a byte.

The frame on my desk is showing a Degas pastel it fetched from
Chicago this morning, sized to the pixel, over an API someone
maintains so that anyone — apparently including a microcontroller —
can ask. Next on its roadmap is the Cleveland Museum of Art, whose
API ranks, by the measures developed here, friendlier than several
already shipped. The queue of museums worth thanking is not getting
shorter.

## Acknowledgments and disclosure

p3a is open source (Apache 2.0) at github.com/fabkury/p3a; the
seven adapters described here are in `components/art_institution/`.
The project — and this article's endpoint probes of 2026-08-07 —
...
revised by the author, who verified the technical claims and takes
sole responsibility for the content. The author has no financial
interest in any product, service, or institution discussed; p3a and
Makapix Club are noncommercial open-source projects; the author has
no affiliation with Anthropic, Cursor, or any museum named.

## About the author

Fabrício Kury (pub@kury.dev, github.com/fabkury) is a biomedical
