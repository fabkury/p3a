...
- **Image rendition:** firmware requests
  `…/full/!720,720/0/default.jpg` — universally supported by the
  Tier-1 IIIF servers surveyed.
...
- **Cover email's last paragraph** offers an off-ramp ("if iiif.io/news
  ...
  of a dead end.
- **Drop-in blurb is fully self-contained** — three museums, one hook
  (Linked Art on-device), one constraint (32 MB PSRAM), license, link.
  Matches the 2–3 sentence length of recent posts.
