...
| SI         | `GET https://api.si.edu/openaccess/api/v1.0/search?api_key={key}&q="{idsId}"&rows=1`                                          | `response.rows[0].title`    | via `smithsonian.js` `getArtist()` (nested freetext)   | via `getDate()` (indexedStructured/freetext)    | `api_key` query (from `/config`)  | yes     |
| CMA        | `GET https://openaccess-api.clevelandart.org/api/artworks/{accession}`                                                        | `data.title`                | `data.creators[0].description`                         | `data.creation_date`                            | none                              | yes     |
| Mia        | `GET https://search.artsmia.org/id:{id}?size=1`                                                                               | `hits.hits[0]._source.title` | `hits.hits[0]._source.artist`                          | `hits.hits[0]._source.dated`                    | none                              | yes     |
