...
| Tool | Purpose |
|------|---------|
| `build.ps1 [-Diag] [-Flash] [-FullClean] [-Port COM5] [-Extra overlay...] [-Suffix name] [-FlashOnly]` | Activates IDF, builds release (`build/`) or diag (`build-diag/`, release sdkconfig + `sdkconfig.diag.defaults`; the generated `build-diag/sdkconfig` is regenerated automatically when any source file is newer, because IDF applies defaults only at creation). `-Extra` adds overlays, `-Suffix` builds into `build-diag-<name>/`. Guards: release `sdkconfig` unchanged, P4 rev-v1.0 lines present, trace on/off as expected, wrap on in release. Prints an `ARM:` line (wrap / IDF patch presence from the map file + sha256). `-Flash` flashes the dev unit (stop any logger first: flashing needs COM5); `-FlashOnly` flashes an existing build dir with esptool and never rebuilds. **Run from PowerShell, not Git Bash**: with `MSYSTEM` set, `idf.py` refuses to build. |
| `arm_reproducer.py RUN --arm stock\|patch\|wrap` | esp-idf #19034 evaluation: checks the device's reported busy-wait variant matches the arm, uploads the bar GIF, dwell 3600 s, runs `sd_experiment.py`, restores settings. |
| `run_audit.py RUN` | Reboot classification (UART `rst:` reason + panic/deliberate markers: external vs firmware vs deliberate) and coverage gaps from puller heartbeats (laptop sleep) → `audit.md`. |
