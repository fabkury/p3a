...
typedef enum {
    ...
    GESTURE_STATE_LONG_PRESS_PENDING, // Finger down, counting to 2.25s for provisioning
...
static void app_touch_task(void *arg)
{
    ...
    const TickType_t long_press_duration = pdMS_TO_TICKS(2250); // 2.25 seconds
    ...
    while (true) {
        ...
        if (pressed && touch_count > 0) {
            ...
            if (gesture_state == GESTURE_STATE_IDLE) {
                ...
                // Check for long press: finger held at same position for 2.25 seconds
