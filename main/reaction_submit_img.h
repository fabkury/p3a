#ifndef REACTION_SUBMIT_IMG_H
#define REACTION_SUBMIT_IMG_H

#include <stdint.h>

extern const int reaction_submit_img_w;
extern const int reaction_submit_img_h;

/**
 * Fast blit the reaction_submit_img image to a destination BGR888 buffer using memcpy.
 *
 ...
 */
void reaction_submit_img_blit_memcpy_bgr888(
    uint8_t *dst,
    int dst_w,
    int dst_h,
...
/**
 * Blit the reaction_submit_img image with alpha blending, scaling, and rotation to a BGR888 buffer.
 *
 ...
 */
void reaction_submit_img_blit_pixelwise_bgr888(
    uint8_t *dst,
    int dst_w,
    int dst_h,
    int dst_stride_bytes,
